...
import { analyzeTopMatches } from "@/lib/ai/helpers/compat";
...
export async function GET(request: NextRequest) {
  try {
    console.log("Matches API route called");
    ...
    if (!session?.user) {
      console.log("No session, attempting to get user from token");
      currentUser = await getUserFromToken(request);

      if (!currentUser) {
        console.log("Authentication required - no session or valid token");
      ...
      // Get User model
      console.log("Looking up user by email", { email: session.user.email });
    ...
    const previouslyMatchedUserIds = (currentUser.previousMatches || [])
      ...
      .map((match: any) => match.userId?.toString() || "");
    ...
    if (listMode) {
      try {
        console.log("List mode activated, preparing to return matches");
        ...
        const candidatesWithFullData = await Promise.all(
          top5Candidates.map(async (score) => {
            ...
            return { user, score, matchDetails: score.matchDetails };
          })
        );
        ...
        // Enhance matches with full user profile data
        const enhancedMatches = await Promise.all(
          top3Matches.map(async (match) => {
            try {
              const userDoc = await User.findById(match.userId).lean();

              if (!userDoc) {
                return match; // Return original match if user not found
              }

              // Use type assertion to treat userDoc as a generic object
              const userProfile = userDoc as any;

              // Return a combined object with original match data and user profile
              return {
                ...match,
                _id: userProfile._id,
                ...
                profession:
                  userProfile.profession || userProfile.occupation || "",
                ...
                personalityQuiz: userProfile.personalityQuiz || null,
                relationshipGoals: userProfile.relationshipGoals || "",
                lifestyle: userProfile.lifestyle || null,
                religion:
                  userProfile.religion ||
                  (userProfile.lifestyle
                    ? userProfile.lifestyle.religion
                    : null) ||
                  "",
                relationshipStatus: userProfile.relationshipStatus || "",
                // Include matchDetails from the AI analysis
                matchDetails: match.matchDetails || {},
              };
            } catch (error) {
              console.error(`Error enhancing match ${match.userId}:`, error);
              return match; // Return original match if there's an error
            }
          })
        );

        // Store the top 3 matches in the user's previousMatches field
        const currentDate = new Date();
        const matchesToStore = top3Matches.map((match) => ({
          userId: new mongoose.Types.ObjectId(match.userId),
          score: match.score,
          reason: match.reason,
          matchDetails: match.matchDetails,
          matchDate: currentDate,
          isViewed: false, // Initialize as not viewed
        }));

        // Add these new matches to the user's previousMatches array
        if (!currentUser.previousMatches) {
          currentUser.previousMatches = [];
        }

        // Only add matches that aren't already in the previousMatches array
        const existingMatchIds = currentUser.previousMatches.map((m: any) =>
          m.userId.toString()
        );
        const newMatches = matchesToStore.filter(
          (m) => !existingMatchIds.includes(m.userId.toString())
        );
        ...
        // Return the matches
        console.log("Successfully found and analyzed matches");
        console.log(
          `Returning ${enhancedMatches.length} matches to client with full profile data`
        );
        console.log("enhancedMatches is: ", enhancedMatches);
        // Add explicit return to ensure we don't continue execution
        return NextResponse.json({
          matches: enhancedMatches,
          total: enhancedMatches.length,
        });
      } catch (error) {
        console.error("Error in list mode:", error);
        return NextResponse.json(
    ...
    // If refresh is true, get the next unviewed match
    if (refresh) {
      try {
        // Check if user has at least one match in their previousMatches
        if (
          !currentUser.previousMatches ||
          currentUser.previousMatches.length === 0
        ) {
          // If no previous matches, forward to the list mode to generate matches
          // We'll catch this and handle it in the front-end
          console.log("No previous matches found, redirecting to list mode");
          return NextResponse.json(
            { redirect: "list", message: "No matches yet" },
            { status: 307 }
          );
        }

        // Get the next unviewed match
        const viewedMatchIds = currentUser.previousMatches
          .filter((m: any) => m.isViewed)
          .map((m: any) => m.userId.toString());

        // Exclude viewed matches and current user
        const excludeIds = [...viewedMatchIds];
        if (currentUser._id) {
          excludeIds.push(currentUser._id.toString());
        }

        // Find all unviewed matches
        const unviewedMatches = currentUser.previousMatches.filter(
          (m: any) => !m.isViewed
        );

        if (unviewedMatches.length === 0) {
          // If all matches have been viewed, generate new matches
          console.log("All matches viewed, need to generate new matches");

          // For now, just return a 404 to indicate no more matches
          // The client can then call the list endpoint to get new matches
          return NextResponse.json(
            { error: "No more unviewed matches" },
            { status: 404 }
          );
        }

        // Get the next unviewed match
        const nextMatchData = unviewedMatches[0];

        // Fetch the complete user data for this match
        const userDoc = await User.findById(nextMatchData.userId).lean();
        ...
        // Create enhanced match with full profile data
        const enhancedMatch = {
          userId: nextMatchData.userId.toString(),
          score: nextMatchData.score || 0,
          reason: nextMatchData.reason || "",
          matchDetails: nextMatchData.matchDetails || {},
          _id: userProfile._id,
          name: userProfile.name || "",
          age: userProfile.age || null,
          gender: userProfile.gender || null,
          profession: userProfile.profession || userProfile.occupation || "",
          education: userProfile.education || "",
          location: userProfile.location || "",
          profileImage: userProfile.profileImage || null,
          bio: userProfile.bio || "",
          ...
          religion:
            userProfile.religion ||
            (userProfile.lifestyle ? userProfile.lifestyle.religion : null) ||
            "",
          relationshipStatus: userProfile.relationshipStatus || "",
        };

        // Mark this match as viewed
        nextMatchData.isViewed = true;
        await currentUser.save();

        return NextResponse.json({ match: enhancedMatch });
      } catch (error) {
    ...
    if (specificUserId) {
      await dbConnect();
      const specificUser = await User.findById(specificUserId).lean();
      ...
      // Find if this user is already in previousMatches
      const existingMatch = currentUser.previousMatches?.find(
        (m: any) => m.userId.toString() === specificUserId
      );

      // If this match is new and should be counted, record it
      if (shouldCountView && !existingMatch) {
        if (!currentUser.previousMatches) {
          currentUser.previousMatches = [];
        }
        ...
        // Add to previousMatches
        currentUser.previousMatches.push(newMatch);
        await currentUser.save();
      } else if (shouldCountView && existingMatch && !existingMatch.isViewed) {
      ...
      // Use type assertion to treat specificUser as a generic object
      const userProfile = specificUser as any;
